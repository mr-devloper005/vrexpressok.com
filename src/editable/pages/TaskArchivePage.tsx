...
const cardBase = 'group block rounded border border-[var(--tk-line)] bg-[var(--tk-surface)] transition duration-300 hover:-translate-y-1 hover:shadow-[0_18px_38px_rgba(15,23,42,0.13)]'

const archiveAdSlots: Partial<Record<TaskKey, 'header' | 'sidebar' | 'in-feed'>> = {
  article: 'header',
  profile: 'sidebar',
  listing: 'in-feed',
}
  ...
  return (
    <EditableSiteShell>
      <main style={taskThemeStyle(task)} className="min-h-screen bg-[var(--tk-bg)] text-[var(--tk-text)]">
        <header className="relative overflow-hidden bg-[#eef0f5]">
          ...
          <div className="relative mx-auto max-w-[var(--editable-container)] px-4 py-10 sm:px-6 lg:px-8">
            <div className="bg-white px-7 py-8 sm:px-12">
            <div className="flex items-center gap-3 text-[11px] font-extrabold uppercase tracking-normal text-[var(--tk-accent)]">
              <span>{theme.kicker}</span>
              <span className="h-1 w-1 rounded-full bg-[var(--tk-accent)] opacity-50" />
              <span className="text-[var(--tk-muted)]">{label}</span>
            </div>
            <h1 className="mt-5 max-w-3xl text-balance text-4xl font-extrabold leading-tight tracking-normal text-[var(--tk-accent)] sm:text-5xl">
              {voice?.headline || `Browse ${label}`}
            </h1>
            <p className="mt-4 max-w-2xl text-base font-semibold leading-7 text-[#071421]">{voice?.description || theme.note}</p>
            {voice?.chips?.length ? (
              <div className="mt-8 flex flex-wrap gap-2.5">
                {voice.chips.map((chip) => (
                  <span key={chip} className="rounded border border-[var(--tk-line)] bg-[#f4f2f2] px-3.5 py-1.5 text-xs font-bold text-[var(--tk-muted)]">{chip}</span>
                ))}
              </div>
            ) : null}
              ...
              <form action={basePath} className="flex items-center gap-2.5">
                <div className="relative">
                  <select
                    ...
                    className="h-11 appearance-none rounded border border-[var(--tk-line)] bg-[var(--tk-surface)] pl-4 pr-10 text-sm font-medium text-[var(--tk-text)] outline-none transition focus:border-[var(--tk-accent)]"
                ...
                <button className="inline-flex h-11 items-center rounded bg-[var(--tk-accent)] px-5 text-sm font-extrabold text-[var(--tk-on-accent)] transition hover:opacity-90">Apply</button>
              </form>
            </div>
            </div>
          </div>
        ...
        {archiveAdSlots[task] ? (
          <div className="mx-auto max-w-6xl px-4 py-6">
            <Ads slot={archiveAdSlots[task]} showLabel eager className="mx-auto w-full" />
          </div>
        ) : null}

        <section className="mx-auto max-w-[var(--editable-container)] px-4 py-5 sm:px-6 lg:px-8">
          {posts.length ? (
            <div className={`bg-white p-5 sm:p-8 ${taskGrid[task]}`}>
              {posts.map((post, index) => <ArchivePostCard key={post.id || post.slug} post={post} task={task} basePath={basePath} index={index} />)}
            </div>
          ) : (
            <div className="mx-auto max-w-xl rounded border border-dashed border-[var(--tk-line)] bg-[var(--tk-surface)] px-8 py-16 text-center">
          ...
          {posts.length ? (
            <nav className="mt-16 flex items-center justify-center gap-3 text-sm">
              {pagination.hasPrevPage ? <Link href={pageHref(basePath, category, page - 1)} className="rounded border border-[var(--tk-line)] bg-white px-5 py-2.5 font-medium transition hover:border-[var(--tk-accent)]">Previous</Link> : null}
              <span className="rounded border border-[var(--tk-line)] bg-[var(--tk-surface)] px-5 py-2.5 font-medium text-[var(--tk-muted)]">Page {page} of {pagination.totalPages || 1}</span>
              {pagination.hasNextPage ? <Link href={pageHref(basePath, category, page + 1)} className="rounded border border-[var(--tk-line)] bg-white px-5 py-2.5 font-medium transition hover:border-[var(--tk-accent)]">Next</Link> : null}
            </nav>
          ) : null}
        </section>
      </main>
